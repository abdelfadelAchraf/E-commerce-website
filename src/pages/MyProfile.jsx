...
const MyProfile = () => {
  const [userData, setUserData] = useState({
    ...
    phone:"+212 63536363",
    address:{
      line1:"56 Cross , Richmood",
      line2:"Circle, Church Road , Rabat"
    },
    gender:"Male",
    dob:"2000-02-18"
  });
  ...
  return (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
       ...
       <p className='font-medium'>Adresse:</p>
       {
        isEdit 
        ? <p>
          <input className='bg-gray-50' type="text" onChange={(e)=>setUserData(prev =>({...prev, address:{...prev.address, line1:e.target.value}}))} value={userData.address.line1} /><br />
       ...
       <div className='mt-10 '>
        {
          isEdit 
          ? <button className='border border-primary px-8 py-2 rounded-full hover:bg-slate-500 hover:text-white transition-all ' onClick={()=>setIsEdit(false)}>Save information</button>
          : <button className='border border-primary px-8 py-2 rounded-full hover:bg-slate-500 hover:text-white transition-all ' onClick={()=>setIsEdit(true)}>Edit</button>
        }
       </div>
    </div>
  )
